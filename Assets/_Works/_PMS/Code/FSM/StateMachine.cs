...
public class StateMachine<T>
{
    ...
    public delegate void StateChangedHandler(StateMachine<T> stateMachine, State<T> newState, State<T> prevState, int layer);
    #endregion
    ...
    private class StateData
    {
        ...
        public List<StateTransition<T>> Transitions { get; private set; } = new();

        public StateData(int layer, int priority, State<T> state)
    ...
    // 즉, State의 Type을 통해 해당 State를 가진 StateData를 찾아올 수 있음, 첫번째 int는 레이어
    ...
    private readonly Dictionary<int, List<StateTransition<T>>> anyTransitionsByLayer = new();
    ...
    #region Setup machine section

    public void Setup(T owner)
    {
        Debug.Assert(owner != null, $"StateMachine<{typeof(T).Name}>::Setup - owner는 null이 될 수 없습니다.");

        Owner = owner;

        AddStates(); //필요한 스테이트를 추가하는 것으로 상속받는 클래스가 구현한다.
        MakeTransitions(); //이것도 상속받을 애가 구현한다.
    ...
    public void SetupLayers()
    {
        foreach ((int layer, var statDatasByType) in stateDatasByLayer)
        {
            ...
            // 해당 레이어에서 우선 순위가 가장 높은 StateData를 찾아옴
    ...
    // StateData가 아닌 State와 layer로 변경하는 오버로딩함수
    private void ChangeState(State<T> newState, int layer)
    {
    ...
    private bool TryTransition(IReadOnlyList<StateTransition<T>> transtions, int layer)
    {
        foreach (var transition in transtions)
        {
            // Command가 존재한다면, Command를 받았을 때만 전이 시도를 해야함으로 넘어감
            // Command가 존재하지 않아도, 전이 조건을 만족하지 못하면 넘어감
            if (transition.TransitionCommand != StateTransition<T>.kNullCommand || !transition.IsTransferable)
                continue;
    ...
    public void Update()
    {
        //업데이트에서는 각 레이어별 AnyTransition을 체크한다.
    ...
    #region addState and make transition

    // Generic을 통해 StateMachine에 State를 추가하는 함수
    // T는 State<EntityType> class를 상속받은 Type이여야함
    public void AddState<StateType>(int layer = 0) where StateType : State<T>
    {
        // Layer 추가, Set이므로 이미 Layer가 존재한다면 추가되지 않음
        layers.Add(layer);

        // Type을 통해 State를 생성
        var newState = Activator.CreateInstance<StateType>();
        newState.Setup(this, Owner, layer);

        //신규 레이어라서 아직 stateDatasByLayer에 추가되지 않은 Layer라면 Layer를 생성해줌
        ...
        Debug.Assert(!stateDatasByLayer[layer].ContainsKey(typeof(StateType)),
            $"StateMachine::AddState<{typeof(StateType).Name}> - 이미 상태가 존재합니다.");

        var stateDatasByType = stateDatasByLayer[layer];
        // StateData를 만들어서 Layer에 추가
        stateDatasByType[typeof(StateType)] = new StateData(layer, stateDatasByType.Count, newState);
    }
    ...
    public void MakeTransition<FromStateType, ToStateType>(int transitionCommand,
        Func<State<T>, bool> transitionCondition, int layer = 0)
        where FromStateType : State<T>
        where ToStateType : State<T>
    {
        ...
        var newTransition = new StateTransition<T>(fromStateData.State, toStateData.State,
    ...
    // MakeTransition 함수의 Enum Command 버전 (가독성을 위함)
    ...
    public void MakeTransition<FromStateType, ToStateType>(Func<State<T>, bool> transitionCondition, int layer = 0)
        where FromStateType : State<T>
        where ToStateType : State<T>
        => MakeTransition<FromStateType, ToStateType>(StateTransition<T>.kNullCommand, transitionCondition, layer);
    ...
    public void MakeAnyTransition<ToStateType>(int transitionCommand,
        Func<State<T>, bool> transitionCondition, int layer = 0, bool canTransitonToSelf = false)
        where ToStateType : State<T>
    {
        var stateDatasByType = stateDatasByLayer[layer];
        // StateDatas에서 ToStateType의 State를 가진 StateData를 찾아옴
        ...
        var newTransition = new StateTransition<T>(null, state, transitionCommand, transitionCondition, canTransitonToSelf);
    ...
    // NullCommand를 넣어서 최상단의 MakeTransition 함수를 호출함
    public void MakeAnyTransition<ToStateType>(Func<State<T>, bool> transitionCondition,
        int layer = 0, bool canTransitonToSelf = false)
        where ToStateType : State<T>
        => MakeAnyTransition<ToStateType>(StateTransition<T>.kNullCommand, transitionCondition, layer, canTransitonToSelf);
    ...
    public bool IsInState<StateType>() where StateType : State<T>
    {
        ...
        {
            if (data.State.GetType() == typeof(StateType))
                return true;
        }
        return false;
    }
    ...
    public bool IsInState<StateType>(int layer) where StateType : State<T>
        => currentStateDatasByLayer[layer].State.GetType() == typeof(StateType);

    // Layer의 현재 실행중인 State를 가져옴
    public State<T> GetCurrentState(int layer = 0) => currentStateDatasByLayer[layer].State;
